...
public class AssertionUtils
{
  ...
  private static void assertJSONMatch(String message, JSONObject expected, JSONObject actual) {
    for (Object key : expected.keySet()) {
      ...
      try {
        ...
        err.printStackTrace();
        throw new AssertionError(format("%s: Entry does not match:\nexpected: '%s',\nactual: '%s',\noriginal error: %s", message, expected, actual, err.getMessage()), err);
      }
    }
  ...
  private static void compareJSONValues(String message, Object e, Object a) {
    ...
    if(e == null ^ a == null) assertEquals(message + ": Null compare failed", e == null ? "null" : "non-null", a == null ? "null" : "non-null");
    ...
    if(e instanceof JSONObject) {
      ...
      if(expectedString.equals(ANY)) {
        ...
        assertTrue(String.format("%s: Actual String does not match pattern. Pattern: %s, actual: %s", message, expectedString, actualString), actualString.matches(expectedString.substring(1, expectedString.length()-1)));
      } else {
