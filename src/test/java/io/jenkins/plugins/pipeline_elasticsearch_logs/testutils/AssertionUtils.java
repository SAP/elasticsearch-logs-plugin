...
/**
 * This class contains methods used to evaluate expected log and Elasticsearch
 * entries.
 */
public class AssertionUtils {
    ...
    private static void assertMatchStringContains(AbstractList<String> expectedEntries,
            AbstractList<String> actualEntries) {
        for (int i = 0; i < expectedEntries.size(); i++) {
            String actual = actualEntries.get(i).trim();
            String expected = expectedEntries.get(i).trim();
            assertTrue(String.format("line %s: \"%s\" not contained in \"%s\"", i + 1, expected, actual),
        ...
        for (Object key : expected.keySet()) {
            ...
            try {
                ...
                throw new AssertionError(
                        format("%s: Entry does not match:\nexpected: '%s',\nactual: '%s',\noriginal error: %s", message,
                                expected, actual, err.getMessage()),
                        err);
            }
        }
    ...
    private static void compareJSONValues(String message, Object e, Object a) {
        ...
        if (e == null ^ a == null)
            assertEquals(message + ": Null compare failed", e == null ? "null" : "non-null",
                    a == null ? "null" : "non-null");
        if (e == null && a == null)
            return;
        assertEquals(String.format(message + ": Different class. expected: %s, actual: %s", e, a), e.getClass(),
                a.getClass());
        if (e instanceof JSONObject) {
            ...
            if (expectedString.equals(ANY)) {
                ...
                assertTrue(
                        String.format("%s: Actual String does not match pattern. Pattern: %s, actual: %s", message,
                                expectedString, actualString),
                        actualString.matches(expectedString.substring(1, expectedString.length() - 1)));
            } else {
