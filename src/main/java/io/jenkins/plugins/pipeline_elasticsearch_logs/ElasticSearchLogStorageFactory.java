...
public class ElasticSearchLogStorageFactory implements LogStorageFactory {
    ...
    private static class ElasticSearchLogStorage implements LogStorage {
        ...
        private AnnotatedLargeText<Executable> tryReadLogFile(Executable build, boolean complete) {
            ...
            AnnotatedLargeText<Executable> annotatedLargeText = new AnnotatedLargeText<Executable>(buf, StandardCharsets.UTF_8, complete,
                    build);
            try {
            ...
            try {
                ...
                if (executable instanceof Executable) {
                    ...
                    if (logFile.length() > 0) {
                        ...
                        return new BrokenLogStorage(
                                new RuntimeException("Elasticsearch plugin does not provide step log data from local files yet."))
